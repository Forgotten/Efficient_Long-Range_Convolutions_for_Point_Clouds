...
if not path.exists(dataFile):
  ...
  if DataType == "Periodic":
    ...
    pointsArray, \
    potentialArray, \
    forcesArray  = genDataPer3DMixed(Ncells, Np, mu1, mu2,Nsamples, minDelta, Lcell,
                                     weight1,weight2)
  else: 
...
for cycle, (epochs, batchSizeL) in enumerate(zip(Nepochs, batchSizeArray)):
  ...
  for epoch in range(epochs):
    ...
    errtrain = tf.sqrt(tf.reduce_sum(tf.square(forcetrain - forces_train[:10,:,:])))/tf.sqrt(tf.reduce_sum(tf.square(forcetrain)))
